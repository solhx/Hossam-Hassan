...
function DustParticles() {
  ...
  // ✅ Only run dust particles on mobile.
  // On desktop, Three.js HeroScene already provides ambient motion.
  // Running both wastes ~2ms/frame on the desktop GPU for no visual gain.
  // On mobile, HeroScene never mounts, so dust particles run alone (cheap).
  ...
  const initParticles = useCallback((width: number, height: number) => {
    // ✅ Fewer particles on mobile — 60 max instead of 100
  ...
  useEffect(() => {
    // ✅ Skip entirely on desktop (Three.js covers it) and reduced motion
    ...
    // ✅ DPR capped at 1 on mobile canvas — halves fill rate vs DPR 2.
    // Dust particles are tiny and blurred — DPR 1 is indistinguishable.
    const dpr = 1;
    ...
    const animate = () => {
      ...
      // ✅ Skip every other frame — effectively 30fps on 60fps device.
      // Dust particles are slow-moving and decorative.
      // Users cannot distinguish 30fps from 60fps for this effect.
      // This halves the CPU cost of the canvas loop on mobile.
      if (frameCount % 2 === 0) {
  ...
  }, [reduced, isMobile, initParticles]);

  // ✅ Don't mount canvas at all on desktop — saves DOM node + memory
